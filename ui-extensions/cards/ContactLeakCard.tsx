/**
 * Contact Leak Card Component
 * CRM card for displaying leak detection insights on Contact records
 */
...
interface ContactLeakCardProps {
  context: {
    objectId: string;
    objectType: 'contact';
    portalId: string;
...
export const ContactLeakCard: React.FC<ContactLeakCardProps> = ({
  context,
  runServerless,
  sendAlert,
}) => {
  ...
  const loadLeaks = useCallback(async () => {
    try {
      ...
      const response = await fetchLeaksForEntity('contact', context.objectId);
      if (response.success) {
        setState(prev => ({
          ...prev,
...
export default ContactLeakCard;
