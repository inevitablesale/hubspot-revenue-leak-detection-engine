/**
 * Company Leak Card Component
 * CRM card for displaying leak detection insights on Company records
 */
...
interface CompanyLeakCardProps {
  context: {
    objectId: string;
    objectType: 'company';
    portalId: string;
...
export const CompanyLeakCard: React.FC<CompanyLeakCardProps> = ({
  context,
  runServerless,
  sendAlert,
}) => {
  ...
  const loadLeaks = useCallback(async () => {
    try {
      ...
      const response = await fetchLeaksForEntity('company', context.objectId);
      if (response.success) {
        setState(prev => ({
          ...prev,
...
export default CompanyLeakCard;
