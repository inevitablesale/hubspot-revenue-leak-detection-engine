/**
 * Ticket Leak Card Component
 * CRM card for displaying leak detection insights on Ticket records
 */
...
export const TicketLeakCard: React.FC<TicketLeakCardProps> = ({
  context,
  runServerless,
  sendAlert,
}) => {
  ...
  const loadLeaks = useCallback(async () => {
    try {
      ...
      const response = await fetchLeaksForEntity('ticket', context.objectId);
      if (response.success) {
        setState(prev => ({
          ...prev,
